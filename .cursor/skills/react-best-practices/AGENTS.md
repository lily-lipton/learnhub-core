...
> **Note:**  
> Adapted for **Vite + React (SPA)** in this project. Skip Next.js / RSC /
> server-only patterns. Use `React.lazy()` instead of `next/dynamic`.

---

## Abstract

Performance optimization guide for React client applications (Vite + SPA). Contains rules across 7 categories, prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Section 3 (Server-Side Performance) is omitted — not applicable to this stack.
